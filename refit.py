...
for layer in range(0, ud_model.model.cfg.n_layers):
    try:
        with open(f"data/probes/{model_name}_layer_{layer}.pkl", "rb") as f:
            probes = pickle.load(f)
        start_probe = max(probes.keys()) + 1
...
# %%
for layer in range(start_probe, ud_model.model.cfg.n_layers):
    ...
    probes[layer] = probe
    next_path = f"data/probes/{model_name}_layer_{layer}.pkl"
    with open(next_path, "wb") as f:
        print(f"Saving probes through layer {layer} to {next_path}")
...
#     sae_comp['_'.join([sae_name, str(sae_width)])] = dict(enc_similarities=enc_similarities, dec_similarities=dec_similarities, all_frac_active=all_frac_active, cfgs=cfgs, max_enc_similarities=max_enc_similarities, max_dec_similarities=max_dec_similarities, enc_null_maxes=enc_null_maxes, dec_null_maxes=dec_null_maxes, n_layers=n_sim_layers, start_layer=0 if sae_name == "gemma_scope" else 1)
...
#         class_results = pd.read_parquet(fpath)
#         acts_results = pd.read_parquet(fpath.replace("class_results", "acts_results"))
...
#             break
#         # stats = plot_precision_recall(acts_results, top_latents=top_latents, save_path=f"figures/sae/{which_sae}/top_{n_top}_latents_evaluation_{model_name}.png")
#     else:
#         class_results, acts_results, stats = evaluate_top_latents_main(
#             ud_model=ud_model,
#             train_loader=train_loader,
...
#         f1_diff = probe_f1_matrix[sae_vals["start_layer"]:] - sae_vals["stats"][which_class_name]["f1"]
...
#         plt.title(f"{probe_type.capitalize()} Probe $-$ {sae_name} SAE {which_class_name} F1 score difference")
#         plt.show()
